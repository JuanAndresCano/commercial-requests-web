...
**Actualización 2026-09-20 — la corrección anterior quedó incompleta.** `isReadyForKamHandoff` sí arregló el número de la tarjeta KPI y el badge de la columna, pero el Kanban del KAM seguía **agrupando las tarjetas** por el `status` real (`en-costeo` completo), no por si estaban realmente listas. Resultado, encontrado por Tomás probando el KAM en vivo: la columna "Lista para Entregar" decía "1" arriba pero mostraba 3 tarjetas idénticas debajo (incluidas las que tenían "Cliente pidió ajustes"), sin ninguna forma de saber cuál de las 3 era la que contaba. Se corrigió de raíz con `kamStageOf()` (`KamCommandCenter.tsx`): una `en-costeo` sin confirmar por el Líder se reclasifica como `en-experto` para efectos de KPI, filtro de tabla **y** agrupación del Kanban — una sola función alimenta las tres vistas, así que "Lista para Entregar" solo puede contener lo que de verdad se puede entregar. De paso se agregó un atajo visual "Revisar y entregar" al pie de cada tarjeta en esa columna (mismo patrón que los botones de acción del tablero del Líder).
