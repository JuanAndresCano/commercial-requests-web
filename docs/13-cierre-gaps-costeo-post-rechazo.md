...
- [ ] En `ProposalCostingModule.tsx`, junto al input manual de "Margen de Contribución ($)", un texto pequeño (no editable, no se guarda): `Referencia: {marginPercent}% de {valor final} = {formatCop(valorFinal * marginPercent / 100)}`. Solo ayuda a comparar visualmente; nunca sobreescribe el campo manual.
...
- [ ] Ampliar la interfaz `NegotiationRound` en `mock-data.ts`: agregar `participantes?: string`, `modalidad?: string`, `horas?: string`, `type?: RequestType`, `necesidad?: string`.
- [ ] En `handleMarkReadyForKam` (`RequestDetail.tsx`), al construir/actualizar la ronda, incluir un snapshot de estos 5 campos tomados de `req` (no de `req.costing`) en el momento del envío.
- [ ] En la sección "Historial de Negociación", si alguno de estos campos difiere respecto a la ronda anterior, mostrarlo como parte del resumen de la ronda (ej. "Participantes: 15-20 → 9-12"). Si no hay ronda anterior con la que comparar (ronda 1), no mostrar diffs, solo el snapshot.
...
- [ ] Cambiar `canEditFullInfo` a: `(isKam && req.kam === user.name && req.status === "nueva") || (role === "lider-producto" && req.productLeader === user.name && req.status === "en-costeo")`.
- [ ] No se requiere ningún cambio adicional de UI para esto — el modal de edición (`isEditingFullInfo`/`fullInfoDraft`) ya es genérico y no está condicionado a un rol específico en su contenido, solo en si aparece el botón "Editar información".
- [ ] Esto se integra solo con el punto 3: si el Líder corrige `necesidad` (u otro campo de alcance) y luego confirma "Enviar a KAM", el snapshot de esa ronda ya va a capturar el valor corregido automáticamente — no hace falta lógica adicional para conectar ambos cambios.
...
- [ ] En `mock-data.ts`: quitar `requiresExternalAdvisor` y `externalAdvisorDetails` de la interfaz `ProposalCosting`. Actualizar la firma de `calculateCosting(...)` quitando esos 2 parámetros, y ajustar todos los call sites existentes en `MOCK_REQUESTS` (son varios, buscar `calculateCosting(`).
...
- [ ] Quitar esos 2 parámetros de `triggerSave(...)` y de todas las llamadas internas del componente.
- [ ] Actualizar `docs/07-gaps-conocidos-y-deuda-tecnica.md`: marcar el gap #11 como ✅ RESUELTO, con una línea explicando la solución (igual que los demás ítems ya resueltos en ese archivo).

## Checklist de verificación
- [ ] `npm run build`, `npm run lint`, `npm run test` limpios.
- [ ] Probar manualmente (leyendo el código): `REQ-2026-0135` ahora debe mostrar su ronda 1 rechazada en el historial apenas se abre, sin tocar nada.
- [ ] Confirmar que un Líder de Producto puede abrir "Editar información" en una solicitud propia en "En Costeo", y que un KAM sigue sin poder hacerlo fuera de "Nueva".
- [ ] Confirmar que el bloque de asesor externo en la tarjeta de costeo ya no tiene ningún control editable, y refleja fielmente lo que dice "Equipo Asignado".
