...
export default function Dashboard() {
  ...
  const stats = (() => {
    if (role === "lider-producto") {
      return [
        { label: "Mis solicitudes asignadas", value: misAsignadas || myRequests.length, icon: Layers, color: "text-[#5454e9]" },
        { label: "Pendientes de docente", value: sinProfesor, icon: AlertCircle, color: "text-[#e9683b]" },
        { label: "En formulación de propuesta", value: enPropuesta, icon: TrendingUp, color: "text-[#865cf0]" },
        { label: "Listas para entregar a KAM", value: listas, icon: CheckCircle2, color: "text-[#4cb979]" },
      ];
    }
    if (role === "profesor") {
      return [
  ...
  return (
    <AppShell>
      <div className="space-y-6">
                          ...
                          <div className="flex items-center justify-end gap-1.5">
                            {role === "lider-producto" && (
                              <Button
                                ...
                                className="h-7.5 px-2.5 text-xs font-medium text-muted-foreground hover:text-foreground border-border hover:bg-secondary/50 gap-1 cursor-pointer"
                                title="Reasignar a otro líder de producto si no corresponde a tu área"
                              >
        ...
        {/* Modal de Reasignación de Líder de Producto */}
        <Dialog
          ...
          <DialogContent className="max-w-md">
            <DialogHeader>
              ...
              <DialogDescription className="text-xs text-muted-foreground">
                Transfiere la gestión de esta solicitud comercial al líder de producto o nodo correspondiente.
              </DialogDescription>
            </DialogHeader>
            ...
            {reassigningRequest && (
              <div className="space-y-4 py-2 text-xs">
                <div className="rounded-lg border border-border bg-secondary/30 p-3 space-y-1.5">
                  ...
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">KAM responsable:</span>
                    <span className="font-medium text-foreground">{reassigningRequest.kam}</span>
                  </div>
                </div>

                <div className="space-y-1.5">
                  <Label htmlFor="new-leader-select" className="text-xs font-semibold text-foreground">
                  ...
                  >
                    <SelectTrigger id="new-leader-select" className="text-xs h-9">
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="reassign-node-select" className="text-xs font-semibold text-foreground">
                    Nodo Temático sugerido
                  </Label>
                  <Select value={selectedNewNode} onValueChange={setSelectedNewNode}>
                    <SelectTrigger id="reassign-node-select" className="text-xs h-9">
                ...
                <div className="space-y-1.5">
                  ...
                  <Textarea
                    id="reassign-notes"
                    rows={2}
                    placeholder="Ej. Esta solicitud trata sobre analítica aplicada al sector financiero y requiere acompañamiento del nodo IA..."
                    value={reassignNotes}
